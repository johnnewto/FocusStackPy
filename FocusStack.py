...
def align_images(images):
    ...
    image1gray = cv.cvtColor(images[0],cv.COLOR_BGR2GRAY)
    ...
    for i in range(1,len(images)):
        ...
        newimage = cv.warpPerspective(images[i], hom, (images[i].shape[1], images[i].shape[0]), flags=cv.INTER_LINEAR)
        ...
        cv.imwrite("aligned/aligned{}.png".format(i), newimage)
...
def focus_stack(unimages):
    ...
    for i in range(len(images)):
        print ("Lap {}".format(i))
        laps.append(doLap(cv.cvtColor(images[i],cv.COLOR_BGR2GRAY)))
