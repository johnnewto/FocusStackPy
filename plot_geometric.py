...
tform1 = tf.SimilarityTransform(scale=1, rotation=math.pi/4,
                               translation=(text.shape[0]/2, -100))

tform2 = tf.SimilarityTransform(scale=0.5, rotation=math.pi/3,
                               translation=(text.shape[0]/2, -100))

rotated1 = tf.warp(text, tform1)
back_rotated1 = tf.warp(rotated1, tform1.inverse)

rotated2 = tf.warp(text, tform2)
back_rotated2 = tf.warp(rotated2, tform2.inverse)

both = tf.warp(text, tform1+tform2 + tform2.inverse+tform2.inverse)

fig, ax = plt.subplots(nrows=4)

ax[0].imshow(text, cmap=plt.cm.gray)
ax[1].imshow(rotated1, cmap=plt.cm.gray)
ax[2].imshow(back_rotated1, cmap=plt.cm.gray)
ax[3].imshow(both, cmap=plt.cm.gray)
